#!/usr/bin/env python3
"""
Convert Creeds.json format to Westminster Standards format with actual scripture text
"""
...
from fetch_scriptures import ScriptureFetcher
...
def convert_confession(creeds_data: Dict[str, Any], scripture_fetcher: ScriptureFetcher) -> Dict[str, Any]:
    """Convert Creeds.json confession format to Westminster Standards format with actual scripture text"""
    ...
    for item in creeds_data.get("Data", []):
        ...
        for section in sections:
            ...
            # Convert proofs to the expected format with actual scripture text
            proof_texts = []
            for proof in proofs:
                ...
                for ref in references:
                    ...
                    # Get actual scripture text
                    scripture_text = scripture_fetcher.get_verses_text(formatted_ref)
                    
                    proof_texts.append({
                        "reference": formatted_ref,
                        "text": scripture_text if scripture_text else f"[{formatted_ref}]"
                    })
...
def convert_catechism(creeds_data: Dict[str, Any], scripture_fetcher: ScriptureFetcher, is_larger: bool = False) -> Dict[str, Any]:
    """Convert Creeds.json catechism format to Westminster Standards format with actual scripture text"""
...
def main():
    """Main conversion function with scripture text"""
    
    print("Initializing scripture fetcher...")
    scripture_fetcher = ScriptureFetcher()
    ...
    for file_info in files_to_convert:
        print(f"\nConverting {file_info['type']} with scripture text...")
        ...
        if file_info["type"] == "confession":
            converted_data = convert_confession(creeds_data, scripture_fetcher)
        elif file_info["type"] == "shorter_catechism":
            converted_data = convert_catechism(creeds_data, scripture_fetcher, is_larger=False)
        elif file_info["type"] == "larger_catechism":
            converted_data = convert_catechism(creeds_data, scripture_fetcher, is_larger=True)
        
        # Write the converted file
        with open(file_info["output"], "w", encoding="utf-8") as f:
            json.dump(converted_data, f, indent=2, ensure_ascii=False)
        
        print(f"✓ Converted {file_info['type']} to {file_info['output']} with actual scripture text")
    
    print("\nConversion complete! All files have been updated with data from Creeds.json and actual KJV scripture text")

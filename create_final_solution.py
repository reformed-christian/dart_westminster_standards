...
REFERENCES_PATH = "assets/westminster_larger_catechism_references.json"
OUTPUT_PATH = "assets/westminster_larger_catechism_final_correct.json"
...
def load_references():
    """Load the references file to get correct footnote numbers"""
    with open(REFERENCES_PATH, 'r', encoding='utf-8') as f:
        return json.load(f)

def extract_from_pdf_with_references():
    """Extract from PDF and use references file for correct footnote numbering"""
    doc = fitz.open(PDF_PATH)
    references = load_references()
    
    # Get all available footnote numbers from references
    available_footnotes = set(int(k) for k in references.keys())
    print(f"Available footnotes in references: {len(available_footnotes)}")
    print(f"Footnote range: {min(available_footnotes)} - {max(available_footnotes)}")
                ...
                for line in block.get('lines', []):
                    ...
                    if question_match:
                        ...
                        if current_question is not None:
                            ...
                            clauses = extract_clauses_with_references(current_spans, available_footnotes)
...
def extract_clauses_with_references(spans, available_footnotes):
    """Extract clauses from spans, using only valid footnote numbers from references"""
    clauses = []
    current_clause = ""
    ...
    for span in spans:
        ...
        if text.isdigit() and font_size < 9.0:
            ...
            # Only use footnote numbers that exist in the references
            if footnote_num in available_footnotes:
...
if __name__ == "__main__":
    print(f"Extracting catechism with correct footnote references...")
    questions = extract_from_pdf_with_references()
